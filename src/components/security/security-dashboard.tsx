...
export default function SecurityDashboard() {
  ...
  const cargarDashboardData = async () => {
    ...
    try {
      ...
      const response = await fetch('https://parcial1-backend.railway.app/api/authz/seguridad/dashboard/', {
        headers: {
  ...
  const cargarActividadBackend = async () => {
    ...
    try {
      ...
      const response = await fetch('https://parcial1-backend.railway.app/api/authz/seguridad/acceso/logs/?limit=10', {
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}`,
          'Content-Type': 'application/json',
        },
