/**
 * Hook personalizado para gestión de usuarios - BACKEND FUNCIONANDO ✅
 * Conecta con http://127.0.0.1:8000/api/personas/ 
 * 
 * CAMPOS BÁSICOS: id, nombre, apellido, nombre_completo, documento_identidad, telefono, email, tipo_persona, activo
 * CAMPOS ADICIONALES: unit_number, numero_unidad, unidad, profession, profesion
 * 
 * ROLES MAPEADOS:
...
export function useUsuarios(): UseUsuariosReturn {
  ...
  const fetchUsuarios = useCallback(async () => {
    try {
      ...
      if (!token) {
        console.warn('⚠️ No hay token, usando datos de fallback');
        ...
        setError('No estás autenticado. Mostrando datos de ejemplo.');
        return;
      }
      ...
      if (!response.ok) {
        ...
        if (response.status === 500) {
          ...
          console.error('🛡️ Usando datos de fallback mientras se resuelve el problema...');
          ...
          setError('⚠️ Error interno del servidor. Mostrando datos de ejemplo.');
      ...
      // Log para ver estructura de una persona individual
      if (Array.isArray(data) && data.length > 0) {
        ...
        console.log('🔍 Campos disponibles en primera persona:', Object.keys(data.results[0]));
        console.log('🔍 Ejemplo persona completa:', data.results[0]);
      }
      ...
      // Función para mapear tipo_persona a rol descriptivo
      const mapearTipoPersonaARol = (tipo_persona: string) => {
        const rolesMap: Record<string, { id: number; nombre: string; descripcion: string }> = {
          'administrador': { id: 1, nombre: 'Administrador', descripcion: 'Administrador del sistema' },
          'propietario': { id: 2, nombre: 'Propietario', descripcion: 'Propietario de unidad' },
          'inquilino': { id: 3, nombre: 'Inquilino', descripcion: 'Inquilino de unidad' },
          'seguridad': { id: 4, nombre: 'Seguridad', descripcion: 'Personal de seguridad' }
        };
        
        return rolesMap[tipo_persona?.toLowerCase()] || rolesMap['administrador'];
      };

      // Transformar datos del backend al formato del frontend
      // Campos disponibles: id, nombre, apellido, nombre_completo, documento_identidad, telefono, email, tipo_persona, activo
      const usuariosTransformados: UsuarioSistema[] = personasArray.map((persona: any) => {
        ...
        persona: {
          ...
          genero: (persona.genero === 'masculino' || persona.genero === 'femenino' || persona.genero === 'otro') 
            ? persona.genero : 'otro',
          pais: 'Bolivia',
          tipo_persona: (persona.tipo_persona && ['inquilino', 'propietario', 'seguridad', 'administrador'].includes(persona.tipo_persona)) 
            ? persona.tipo_persona : 'administrador',
          direccion: persona.direccion || 'N/A',
          edad: persona.edad || 0,
          nombre_completo: persona.nombre_completo || `${persona.nombre || 'N/A'} ${persona.apellido || 'N/A'}`,
      ...
      console.log(`🛡️ useUsuarios: ${fallbackUsuarios.length} usuarios de fallback cargados por error`);
    } finally {
      setLoading(false);
    }
  }, [filters]);
