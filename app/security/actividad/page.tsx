...
export default function ActividadPage() {
  ...
  const cargarDatosActividad = async () => {
    ...
    try {
      ...
      const logsResponse = await fetch('http://localhost:8000/api/authz/seguridad/acceso/logs/?limit=50', {
        headers: {
      ...
      if (logsResponse.ok) {
        ...
        const actResponse = await fetch('http://localhost:8000/api/seguridad/actividad/reciente/?limit=50', {
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('access_token') || ''}`,
            'Content-Type': 'application/json',
          },
